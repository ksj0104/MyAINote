  ...
  const LEVELS = [
    /* ═══════════ SERIES 1 — 각성 (기초) ═══════════ */
    {
      ...
      brief: [
        ...
        '0xMOTHER> 천천히 알려줄게. 디렉터리(directory)는 그냥 *폴더*야. 지금 너는 /home/guest 라는 네 방 안에 서 있는 거고.',
        '0xMOTHER> 터미널(검은 명령창)의 첫 언어는 둘이야. `ls`(list) — 방 안에 뭐가 있는지 *목록*을 보는 눈. `cat` — 파일을 펼쳐 *내용*을 읽는 손.',
        '0xMOTHER> 명령은 한 줄 치고 Enter. 떨어도 좋다, 멈추지만 마. 그 파일 안에 네 첫 접속 코드가 있다.'
      ].join('\n'),
    ...
    },
    {
      ...
      brief: [
        ...
        '0xMOTHER> 유닉스에선 이름이 점(.)으로 시작하는 파일은 평범한 목록에서 사라진다 — 이걸 *숨김 파일*이라 불러. 설정·비밀은 늘 거기 숨어.',
        '0xMOTHER> 명령 뒤에 붙는 `-a` 같은 걸 옵션(플래그)이라 해 — 명령의 동작을 바꾸는 스위치야. `ls -a` 의 -a 는 "all", 숨김까지 다 보여줘.',
        '0xMOTHER> 찾을 건 .env 파일. env=environment(환경설정), 앱이 쓸 비밀번호·키를 담아두는 곳이지. 가려진 걸 의심하는 법부터 배워라.'
      ].join('\n'),
    ...
    },
    {
      ...
      brief: [
        '0xMOTHER> 시스템 로그 하나를 빼왔다. 로그(log)는 시스템이 한 일을 시간순으로 적어둔 *일지*야 — 수만 줄짜리 건초더미지.',
        '0xMOTHER> 눈으로 훑을 생각 마라. `grep` 은 해커의 자석이야: "찾을 단어 + 파일" 순서로 써 → `grep HELIOS system.log`. 그 단어가 든 줄만 끌어올린다.',
        '0xMOTHER> 찾을 건 DB(데이터베이스) 서버의 IP 주소. IP는 10.0.1.9 처럼 점으로 나뉜 네 칸 숫자 — 네트워크 상의 집 주소다. 그걸 캐내.'
      ].join('\n'),
    ...
    },
    {
      ...
      brief: [
        '0xMOTHER> 정찰 스크립트를 하나 보냈는데(스크립트=명령을 모아 한 번에 실행하는 작은 자동화 파일), 어떤 바보가 실행 권한을 막아뒀군.',
        '0xMOTHER> 유닉스의 모든 파일엔 세 권한이 붙어 — 읽기(r)·쓰기(w)·실행(x). x 가 없으면 코드여도 한 줄도 안 돌아.',
        '0xMOTHER> `chmod +x` = change mode, "실행(execute) 권한을 더해라". 실행할 땐 `./start.sh` — 앞의 `./` 는 "바로 이 폴더의" 라는 뜻이다. 권한은 곧 주인의 자격이야.'
      ].join('\n'),
    ...
    },
    {
      ...
      brief: [
        ...
        '0xMOTHER> base64 는 암호화(키로 잠그기)가 아니라 *인코딩* — 데이터를 안전하게 실어 나르려고 갈아입힌 옷일 뿐이야. 키 없이도 누구나 벗긴다.',
        '0xMOTHER> `base64 -d` — 여기서 `-d` 는 decode(되돌려 해독)야. 반대로 그냥 `base64` 는 encode(부호화)지. 안에 관리자 계정 이름이 있다. 암호와 인코딩을 평생 헷갈리지 마라.'
      ].join('\n'),
    ...
    },
    {
      ...
      brief: [
        ...
        '0xMOTHER> DNS는 도메인 이름(helios.corp)을 컴퓨터 주소(IP)로 바꿔주는 인터넷 *전화번호부*야. 그걸 관리하는 서버가 네임서버고.',
        '0xMOTHER> 네임서버에 "존(zone, 그 도메인 명단) 전체를 복사해줘"라고 조르는 게 AXFR 트랜스퍼야. 제대로 막지 않으면 내부 서브도메인이 통째로 샌다.',
        '0xMOTHER> 서브도메인 = 본 도메인 앞에 붙는 하위 주소(dev-test.helios.corp 처럼). `dig axfr` 로 존을 떠서, 밖으로 노출된 *개발용* 서브도메인 하나를 찾아 — 그게 입구다.'
      ].join('\n'),
    ...
    },
    {
      ...
      brief: [
        ...
        '0xMOTHER> 포트(port)는 한 컴퓨터에 난 수많은 *문(번호)* 이야. 서비스마다 쓰는 문이 정해져 있어 — 웹(HTTP)은 보통 80번 문이지.',
        '0xMOTHER> `nmap` 은 해커의 청진기 — 포트를 하나씩 두드려 열렸는지(open) 닫혔는지 들어준다. 스캔 결과(열린 포트 목록) 중 *웹(HTTP)* 으로 들어가는 문 번호가 우리 통로다.'
      ].join('\n'),
    ...
    },
    {
      ...
      brief: [
        ...
        '0xMOTHER> 정규식(regex)은 "이런 모양의 글자"를 묘사하는 패턴 언어야. `[a-z.0-9_]+@helios.corp` 는 "글자들 + @helios.corp" 모양을 뜻해.',
        '0xMOTHER> `grep -oE` 의 -o 는 "맞은 *조각*만", -E 는 "확장 정규식 사용". 거기에 `>` 를 붙이면 리다이렉션 — 화면에 나올 결과를 *파일로 돌려보내* 저장한다.',
        '0xMOTHER> @helios.corp 이메일만 추려 `> emails.txt` 로 저장해. 이 명단이 우리의 첫 무기다.'
      ].join('\n'),
    ...
    },
    {
      ...
      brief: [
        ...
        '0xMOTHER> FTP는 파일을 주고받는 옛 프로토콜(서비스)야 — 오래된 만큼 구멍도 많지. 그 FTP의 정확한 버전 문자열을 캐내라.',
        '0xMOTHER> 취약점(vulnerability)은 특정 버전의 *알려진 약점*. 전 세계가 CVE 라는 번호로 목록화해 둬 — 버전만 알면 약점도 안다.'
      ].join('\n'),
    ...
    },
    {
      ...
      brief: [
        ...
        '0xMOTHER> 표적 대역은 10.0.1.0/24. 끝의 `/24`는 "대역(range)" 표기 — 10.0.1.0부터 .255까지 한 동네 IP 전부를 뜻해. `nmap 10.0.1.0/24` 로 동네를 통째로 훑어 FTP 열린 호스트를 찾아.',
        '0xMOTHER> `hydra` 는 사전(wordlist, 흔한 비번 목록)을 하나씩 대입하는 도구 — 이게 사전 공격(dictionary attack)이야. (모든 조합을 무작정 넣는 건 브루트포스.)',
