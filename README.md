...
부팅 화면에서 **아무 키나 누르면** 타이틀 메뉴가 뜹니다. 보통의 게임처럼 **새 게임 / 이어하기 = 스토리(시나리오)** 진행이고, 학습·코드랩·워게임은 **그 외 모드** 서브메뉴로 분리되어 있습니다. 게임 중 언제든 `menu` 로 타이틀 복귀.

**조작:** 모든 메뉴는 **↑/↓ 방향키로 항목을 옮기고 Enter 로 선택**합니다(선택 카드가 하이라이트). 숫자(1~4)나 이름을 직접 입력해도 됩니다.
...
css/style.css       "NULLSEC//DECK" 택티컬 OS 셸 — 듀얼톤(차가운 강철 크롬 + 따뜻한 인광 시그널), 코너 브래킷 패널, CRT 효과, 3색 테마
js/filesystem.js    가상 유닉스 파일시스템 (트리·권한·경로해석·쓰기)
js/commands.js      명령어 인터프리터 60종 (ls,grep,nmap,ssh,xor,hydra,scp ...)
...
js/game.js          게임 엔진 (모드 라우팅/진행/저장/원격접속/명령실행)
js/terminal.js      터미널 UI (부팅·사운드·미션패널·메뉴·워게임보드·코드랩)
```

## 검증

`node playtest`(개발용 하니스)로 **시나리오 34 · 워게임 6 · 코드랩 4 + 학습(99명령)·STANCE 분기·교신·메뉴·미션선택**이
모두 의도한 풀이로 클리어 가능함을 확인했습니다(총 **99 검사 PASS**).
